...
CoCo supports Windows x64 and macOS Apple Silicon. The Electron desktop app embeds the web client and server. The web client also supports local development and testing. Mobile, marketing, and hosted relay deployment code are not maintained in this fork. Shared remote-environment support remains intact.
...
- Upon request, user-visible frontend changes should get one integrated pass in a real client: `test-t3-app` for the web client. The primary agent does this once after integrating. Subagents do not launch their own dev servers. Ask permission before doing computer use or spinning up browsers.
...
- `.repos/effect-smol/` - read-only Effect guide and examples, not a full source checkout. Never import them into runtime code. See `PROVENANCE.md` there for source identity and manual refresh instructions. Use the installed dependency or an external version-matched checkout for implementation-source inspection.
