...
import * as Broker from "./V0AgentBroker.ts";
import { registration } from "./V0AgentTools.ts";
...
const tool = {
  ...
  description: "Revise a prototype",
  inputSchema: {
    type: "object",
...
const call = (provider: string, v0 = true) =>
  Effect.gen(function* () {
    const server = yield* McpServer.McpServer;
    return yield* server
      .callTool({ name: `v0_${tool.name}`, arguments: { chatId: "TEAM-1" } })
      .pipe(
        ...
        Effect.provideService(McpInvocationContext, {
          ...
          capabilities: new Set(v0 ? (["v0"] as const) : []),
          issuedAt: 1,
        }),
      );
  });
...
it.effect(
  ...
  () =>
    Effect.scoped(
      Effect.gen(function* () {
        const broker = yield* Broker.V0AgentBroker;
        const server = yield* McpServer.McpServer;
        ...
        expect(
          server.tools.find((entry) => entry.tool.name === `v0_${tool.name}`)?.tool,
        ).toMatchObject({
          ...tool,
          name: `v0_${tool.name}`,
