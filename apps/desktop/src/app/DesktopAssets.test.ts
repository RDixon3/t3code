  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/coco/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/coco\/icon\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/coco\/icon\.png$/);
